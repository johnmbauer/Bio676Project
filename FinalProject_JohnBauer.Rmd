...
```{r, fig.width=12, fig.height=6, fig.align='center'}
NEON_MAGs_metagenomes_chemistry_Alaska %>%
     ggplot(aes(x=`Phylum`,y=`soilInWaterpH`, color=`Phylum`), size=1)+geom_point(show.legend = FALSE)+facet_wrap(vars(Site.x), scales="free_y", ncol=3)+scale_y_continuous(limits=c(0,14),breaks =rep(0:14)) +labs(title="Soil pH", y="Soil pH", x="Site")+theme_classic()+John_theme
```
**Figure 22:**
...
NEON_MAGs_metagenomes_chemistry_TOOL %>%
     ggplot(aes(y=`soilTemp`,x=`Phylum`, color=`Phylum`), size=1)+geom_point(show.legend=FALSE)+scale_y_continuous(limits=c(0,50)) +labs(title="Soil pH", y="Soil pH", x="Site")+theme_classic()+theme(axis.text.x=element_text(angle=45, vjust=1, hjust=1))
